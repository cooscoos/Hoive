// cargo benchmark
// We'll compare 5 methods of finding mutual elements in two collections
...
fn count_neighbours_2<T:Coord>(board: &Board<T>, position: (i8, i8, i8)) -> usize {
    ...
    let neighbour_hexes = neighbour_tiles(position).into_iter().collect::<BTreeSet<_>>();

    // If any of these neighbouring hex coords also appear in the board's list of chips, it means they're a neighbouring chip
    let chip_positions = get_placed_positions(board).into_iter().collect::<BTreeSet<_>>();
...
pub fn count_neighbours_4<T:Coord>(board: &Board<T>, position: (i8, i8, i8)) -> usize {
    ...
    neighbour_hexes.iter().filter(|v| chip_positions.contains(v)).count()
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("double_for_loop", |b| b.iter(|| count_neighbours(&board, position)));     // slowest
    c.bench_function("btree_intersect", |b| b.iter(|| count_neighbours_2(&board, position))); // 4x improvement
    c.bench_function("btree_always", |b| b.iter(|| count_neighbours_3(&board, position)));    // similar, one of the tidiest options
    c.bench_function("btree_always_filter", |b| b.iter(|| count_neighbours_4(&board, position)));    // same again
    c.bench_function("hashset_always_filter", |b| b.iter(|| count_neighbours_5(&board, position)));    // same again
}
