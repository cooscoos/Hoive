/// Contains the structs used by the client (with some of these mirrored on the server)
...
/// User mimics the User struct found in the server's version of models.rs
/// In the client, it doesn't need the trait Serialize.
