...
use super::comps::{Chip,Team,starting_chips};
...
impl History {
    ...
    pub fn save(&self, filename: String) -> std::io::Result<()> {
        ...
        let btree_history = self.history_map.clone().into_iter().collect::<BTreeMap<u32, (Chip,(i8,i8))>>();
        ...
        for (turn, (chip, position)) in btree_history.into_iter() {
            writeln!(&mut file, "{},{:?},{},{},{}",turn,chip.team,chip.name,position.0,position.1)?;
        }
        Ok(())
    }
...
fn load_moves(filename: String) -> std::io::Result<Vec<(Team,String, i8,i8)>> {

    let file = File::open(format!("./saved_games/{}.csv",filename))?;
...
pub fn emulate<T:Coord>(board: &mut Board<T>, filename:String) {

    // Load the moves from the file
    let history_list = match load_moves(filename) {
        Ok(values) => values,
...
fn convert_static(chip_string: String) -> &'static str {
    ...
    let chip_names = chips.into_iter().map(|(c,v)| c.name).collect::<Vec<&str>>();

    // find the chip name that matches the chip_string and return that chip's name
    let matched = chip_names.into_iter().filter(|n| n.to_string() == chip_string).collect::<Vec<&str>>();
    matched[0]
