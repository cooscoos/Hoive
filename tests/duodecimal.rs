...
fn duodecimal_from_decimal() {
    ...
    let result = numbers.into_iter().map(|v| funcs::decimal_to_duo(v)).collect::<Vec<String>>();
...
fn duodecimal_to_decimal() {
    ...
    let result = numbers.into_iter().map(|d| funcs::duo_to_decimal(d)).collect::<Vec<usize>>();
