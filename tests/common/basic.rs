...
pub fn first_turn<T: Coord>(board: &mut Board<T>) {
    ...
    let move_status = pmoore::try_move(board, "a1", Team::Black, (0, 0, 0));
...
pub fn occupied<T: Coord>(board: &mut Board<T>) {
    ...
    pmoore::try_move(board, "a1", Team::Black, (0, 0, 0));
    let move_status = pmoore::try_move(board, "a2", Team::Black, (0, 0, 0));
...
pub fn to_the_moon<T: Coord>(board: &mut Board<T>) {
    ...
    assert_eq!(
        ...
        pmoore::try_move(board, "a2", Team::Black, (0, 0, 8))
    );
}
